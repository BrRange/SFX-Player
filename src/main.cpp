...
#define DEBUG 0
#if DEBUG
    constinit static size_t memoryCount = 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow){
    ...
    memeTime = []{
        if(steady_clock::now() - timeSinceMeme >= milliseconds(waitTime)){
            size_t checkNum = 10ull * RNG::intRange(RAND_MAX);
            waitTime = memeController::playMeme() + checkNum;
            timeSinceMeme = steady_clock::now();
        }
    };
